...
const ApiMedicSymptoms = ({ navigation }) => {
  ...
  const fetchSymptoms = async () => {
    try {
      const response = await fetch(
        'https://healthservice.priaid.ch/symptoms?token=YOUR_API_KEY&format=json&language=en-gb'
      );
  ...
  const handleDiagnosis = async () => {
    ...
    try {
      ...
      const apiUrl = `https://healthservice.priaid.ch/diagnosis?token=YOUR_API_KEY&format=json&language=en-gb&${queryString}`;
